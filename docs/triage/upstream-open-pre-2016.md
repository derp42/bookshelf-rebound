...
| [#730 `model.query().fetch()` does not limit the fetch query](https://github.com/bookshelf/bookshelf/issues/730) | Query correctness | P1 | ALREADY_FIXED | Commit `1780d92` makes an explicit query authoritative instead of adding unrelated model attributes, including custom-ID and loaded-JSON cases. Commit `324d628` separately rejects unsafe object/array-only implicit fetch constraints. | Regressions: `test/unit/sync.js` explicit-query and unsafe-attribute cases. |
...
| [#1075 Count on a `belongsToMany` relation](https://github.com/bookshelf/bookshelf/issues/1075) | Relation correctness | P1 | ALREADY_FIXED | Commit `a081854` joins and constrains plain `belongsToMany` counts through the pivot instead of returning a global target count. Tests cover default keys, custom keys, through relations, and PostgreSQL/MySQL query compilation. | Supersedes the focused intent of upstream PR #2093. |
