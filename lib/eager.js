...
const getParsedAttribute = (model, attribute) => {
  if (Object.prototype.hasOwnProperty.call(model.attributes, attribute)) return model.get(attribute);

  const parsed = model.parse({[attribute]: null});
  const parsedAttribute = parsed && Object.keys(parsed)[0];
  return model.get(parsedAttribute || attribute);
};
const getAttributeUnique = (models, attribute) => _.uniq(_.map(models, (m) => getParsedAttribute(m, attribute)));
...
class EagerRelation extends EagerBase {
  ...
  eagerFetch(relationName, handled, options) {
    ...
    // Skip the query when an inverse relation has no foreign key, but still mark an eager-loaded
    // to-one relation as loaded so it serializes consistently and `related()` retains its model.
    if (relatedData.parentFk === null) {
      if (relatedData.isSingle()) {
        _.each(this.parent, (model) => {
          model.relations[relationName] = model[relationName]();
        });
      }
      return;
    }
  ...
  morphToFetch(relationName, relatedData, options) {
    ...
    const parentsByType = _.groupBy(this.parent, (model) => {
      const type = getParsedAttribute(model, typeColumn);
  ...
  _eagerLoadHelper(response, relationName, handled, options) {
    const relatedData = handled.relatedData;
    const relatedModels = this.pushModels(relationName, handled, response, options);
