...
module.exports = {
  validateTransactionOptions(options) {
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'transaction')) {
      throw new TypeError('Unknown option "transaction". Use "transacting" instead.');
    }

    const transaction = options.transacting;
    if (
      transaction != null &&
      (transaction.isTransaction !== true || !transaction.client || transaction.client.transacting !== true)
    ) {
      throw new TypeError('The "transacting" option must be a Knex transaction.');
    }
  },
  ...
  fetchPage(options) {
    ...
    const count = () => {
      ...
      counter.once(isModel ? 'fetching:collection' : 'fetching', (model, columns, eventOptions) => {
        const innerQuery = eventOptions.query.clone();
        innerQuery.clearOrder();
        innerQuery.clear('limit');
        innerQuery.clear('offset');

        const countQuery = innerQuery.client.queryBuilder().from(innerQuery.as('bookshelf_count')).count('* as count');

        Object.assign(eventOptions.query, countQuery);
      });
