...
ModelBase.prototype.serialize = function (options) {
  ...
  if (options.shallow !== true) {
    let relations = _.mapValues(this.relations, (relation) => {
      const serialized = relation && relation.toJSON ? relation.toJSON(options) : relation;
      const isEmptySingle =
        relation &&
        relation.relatedData &&
        relation.relatedData.isSingle() &&
        relation.isNew() &&
        _.isEmpty(relation.attributes) &&
        _.isEmpty(serialized);

      return isEmptySingle ? null : serialized;
    });
    if (options.omitNew) relations = _.omitBy(relations, _.isNull);
