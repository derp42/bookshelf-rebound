...
const eagerPairingAlias = '__bookshelf_rebound_eager_pairing_key__';
...
const isSimpleColumnProjection = (statement) =>
  !statement.distinct &&
  Array.isArray(statement.value) &&
  statement.value.length > 0 &&
  statement.value.every(
    (value) => _.isString(value) || (_.isPlainObject(value) && _.every(value, (column) => _.isString(column)))
  );
...
const Relation = RelationBase.extend(
  /** @lends Relation.prototype */ {
    ...
    selectConstraints(knex, options) {
      ...
      const columnStatements = _.filter(knex._statements, {grouping: 'columns'});

      if (columnStatements.length === 0) {
        const columns = this.targetTableName + '.*';
        if (this.isJoined()) knex.distinct(columns);
        else knex.columns(columns);
      } else if (resp && !this.isJoined() && columnStatements.every(isSimpleColumnProjection)) {
        // Pair ordinary eager projections without exposing a foreign/target key the caller did
        // not request. Aggregate, raw, distinct, and joined projections retain their existing SQL.
        const pairingColumn = this.isInverse()
          ? this.isForeignKeyTargeted()
            ? this.foreignKeyTarget
            : this.targetIdAttribute
          : this.key('foreignKey');

        knex.column({[eagerPairingAlias]: `${this.targetTableName}.${pairingColumn}`});
        this.eagerPairingAlias = eagerPairingAlias;
      }
    ...
    eagerPair(relationName, related, parentModels, options) {
      ...
      if (this.type === 'morphTo') {
        parentModels = _.filter(parentModels, (m) => {
          const rawMorphKey = this.key('morphKey');
          if (Object.prototype.hasOwnProperty.call(m.attributes, rawMorphKey)) {
            return m.get(rawMorphKey) === this.key('morphValue');
          }

          const parsed = m.parse({[rawMorphKey]: null});
          const morphKey = (parsed && Object.keys(parsed)[0]) || rawMorphKey;
          return m.get(morphKey) === this.key('morphValue');
        });
      }
      ...
      const grouped = _.groupBy(related, (m) => {
        ...
        } else if (this.eagerPairingAlias) {
          key = m.get(this.eagerPairingAlias);
      ...
      _.each(parentModels, (model) => {
        ...
        if (groupedKey != null || this.isSingle()) {
          const relation = (model.relations[relationName] = this.relatedInstance(grouped[groupedKey], options));
          // Recreate relation metadata from the owning model. The temporary
          // eager relation represents the whole parent collection and cannot
          // supply a correct parent foreign key for an individual model.
      ...
      // attributes
      related.map((model) => {
        if (this.eagerPairingAlias) delete model.attributes[this.eagerPairingAlias];
...
const PivotHelpers = {
  ...
  _handler: Promise.method(function (method, ids, options) {
    Helpers.validateTransactionOptions(options);
    const pending = [];
    if (ids == null) {
  ...
  _processModelPivot: Promise.method(function (method, item, options, data, fks) {
    ...
    if (method === 'delete') {
      return joinModel
        .set(fks)
        .query('where', joinModel.format(_.clone(fks)))
        .destroy(_.assign({}, options, {require: false}))
        .then(function () {
          if (item == null) return collection.reset();
          const related = collection.get(item instanceof ModelBase ? item.id : item);
          if (related) collection.remove(related);
        });
    }
