...
function isUnsafeFetchAttribute(attribute) {
  return _.isPlainObject(attribute) || Array.isArray(attribute);
}
...
_.extend(Sync.prototype, {
  ...
  first: Promise.method(function (attributes) {
    ...
    const hasId = Object.prototype.hasOwnProperty.call(attributes, model.idAttribute);
    const hasExplicitWhere = _.some(query._statements, {grouping: 'where'});
    const unsafeAttribute = _.findKey(attributes, isUnsafeFetchAttribute);

    if (!hasExplicitWhere && (unsafeAttribute === model.idAttribute || (!hasId && unsafeAttribute !== undefined))) {
      throw new TypeError(
        `Cannot fetch a model using an object or array value for attribute "${unsafeAttribute}". ` +
          'Use a scalar value or add an explicit query constraint with Model#where.'
      );
    }

    // Once a primary key is available, it is the complete model identity. In
    // particular, do not try to compare JSON attributes populated by a prior
    // fetch or refresh.
    const whereAttributes = hasId || hasExplicitWhere ? {} : attributes;
    const formattedAttributes = model.format(whereAttributes);

    if (hasId && !hasExplicitWhere) {
  ...
  count: Promise.method(function (column) {
    ...
    return Promise.bind(this)
      .then(function () {
        ...
        if (relatedData.isJoined()) relatedData.joinClauses(knex);
        relatedData.whereClauses(knex);
      })
      .then(function () {
        options.query = knex;
  ...
  select: Promise.method(function () {
    ...
    const queryContainsColumns = _(knex._statements).filter({grouping: 'columns'}).some('value.length');
    ...
    return Promise.bind(this)
      ...
      .then(() => {
        // A fetching hook may have selected its own columns after the default was
        // prepared. In that case the hook's projection is the complete selection.
        if (hasDefaultColumns && _(knex._statements).filter({grouping: 'columns'}).some('value.length')) {
          columns = null;
        }
  ...
  update: Promise.method(function (attrs) {
    ...
    if (syncing.id != null && supportsReturning(query.client) && this.options.autoRefresh !== false) {
      query.returning('*');
    }
