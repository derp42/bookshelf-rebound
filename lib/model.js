...
const BookshelfModel = ModelBase.extend(
  {
     ...
     *   be saved and its data fetched with a single query. Updates without a model
     *   id are not refreshed because a broad query does not identify one row.
     * @fires Model#saving
        ...
        .then(function (method) {
          ...
          return this.triggerThen(
            ...
            .then(function (resp) {
              ...
              const hasDeterministicResponse = method === 'insert' || this.id != null;
              const isEmptyResponse = resp == null || resp === 0 || (Array.isArray(resp) && resp.length === 0);

              // After a successful database save, the id is updated if the model was created
              if (method === 'insert' && this.id == null && this.idAttribute != null && !isEmptyResponse) {
                let updatedAttrs;
              ...
              } else if (method === 'update' && isEmptyResponse) {
                if (options.require !== false) {
                  throw new this.constructor.NoRowsUpdatedError('No Rows Updated');
                }
              } else if (isObjectResponse && hasDeterministicResponse) {
                Object.assign(this.attributes, this.parse(resp[0]));
              }

              if (isEmptyResponse) return resp;
              if (isObjectResponse && hasDeterministicResponse) return this;
              if (options.autoRefresh === false) return this;
              if (this.idAttribute == null || this.id == null) return this;
